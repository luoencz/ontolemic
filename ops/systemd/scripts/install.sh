...
# Install a systemd unit from template by envsubst into /etc/systemd/system
# Usage: [ENV_FILE=path] [-e path] [-n unit_name]
...
if [[ -z "${UNIT_NAME:-}" ]]; then
    echo "UNIT_NAME is required (e.g., inner-cosmos-backend.service). Provide via env or -n." >&2
    exit 1
fi

TARGET_PATH="/etc/systemd/system/${UNIT_NAME}"

# Discover required template variables
